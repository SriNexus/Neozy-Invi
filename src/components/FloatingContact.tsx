 ...
 * (bottom-right). Appears the same moment `ScrollCue` does, so it must
 * read as a quiet utility mark, never a website button: no glass pills,
 * no coloured chips, no backdrop-blur — bare icons/text with the same
 * drop-shadow legibility treatment as `SoundToggle` and `ScrollCue`.
 */
...
export default function FloatingContact({ contact }: { contact: ContactData }) {
  ...
  return (
    ...
    >
      {/* Expanded actions — bare marks, no pill/chip background */}
      <div
      ...
      {/* Toggle — a bare icon, same quiet treatment as SoundToggle */}
      <button
        type="button"
        onClick={() => setOpen(!open)}
        aria-label={open ? "Close contact options" : "Open contact options"}
        aria-expanded={open}
        className="flex items-center justify-center w-9 h-9 active:scale-95"
        style={{
